...
@router.get('/{_id}', dependencies=[Depends(get_user_from_token)], status_code=status.HTTP_200_OK,
            summary='получение одного аккаунта')
async def account_by_id(_id: int, manager: DbManagerDep):
    account = await manager.read(model=Account, ident_val=_id)
    return account
...
@router.post('', dependencies=[Depends(get_user_from_token)], status_code=status.HTTP_201_CREATED,
...
@router.delete('/{_id}', dependencies=[Depends(get_user_from_token)], status_code=status.HTTP_200_OK,
               summary='удаление одного аккаунта')
async def delete_account_by_id(_id: int, manager: DbManagerDep):
    await manager.delete(model=Account, ident=_id)
    return _id
